...
class Recipe {
  ...
  static async getRated(username) {
    ...
    const ratedRecipes = res.rows.map(recipe => {
      return {
        id: recipe.recipeId,
  ...
  static async rate(recipeId, username, rating) {
    ...
    if (checkIfExists.rows.length === 0) {
      await db.query(
        `INSERT INTO recipe_rated_by (recipe_id, rated_by, rating)
         VALUES ($1, $2, $3)
    ...
    await Recipe.updateRating(recipeId, rating);

    return { id: recipeId, rating };
  }

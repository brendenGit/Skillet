...
describe("POST /rate", function () {
  test("works", async function () {
    ...
    expect(resp.body).toEqual({ "rating": 5, "id": 2 })
...
describe("GET /:username/saved", function () {
  test("works for correct user", async function () {
    const resp = await request(app)
...
describe("GET /:username/rated", function () {
  test("works for correct user", async function () {
    const resp = await request(app)
      .get("/recipes/u1/rated")
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({ "ratedRecipes": [{ "id": 1, "rating": 4 }, { "id": 2, "rating": 2 }] });
  });

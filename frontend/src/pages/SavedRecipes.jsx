...
export default function SavedRecipes() {
    ...
    const [buttonDisable, setButtonDisable] = useState(user.savedRecipes.length <= 3);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const data = await Promise.all(user.savedRecipes.slice(0, 3).map(recipeId => {
    ...
    const loadMore = async () => {
        if (offset !== null && offset !== undefined) {
            const data = await Promise.all(user.savedRecipes.slice(offset, (offset + 3)).map(recipeId => {
                return skilletApi.getRecipe(recipeId);
            }));
